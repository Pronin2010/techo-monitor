...
// GET /api/meshtastic/script — download the Python bridge script
    ...
    while running:
        try:
            ...
            for node_num, node in interface.nodes.items():
                ...
                # Если usbPower не пришёл из deviceMetrics, fallback по напряжению
                ...
                print(f"  [{timestamp_debug}] {node_entry['shortName']:4s} {bat_str} {volt_str}{usb_str} snr={snr:.1f} rssi={rssi} {gps}")

            if debug:
                print(f"[DEBUG] node_rssi: {node_rssi}")
                print(f"[DEBUG] node_position: {node_position}")
...
`
  ...
  return new Response(script, {
    headers: {
      ...
      'Content-Disposition': 'attachment; filename="techo-bridge.py"',
    },
  })
}
