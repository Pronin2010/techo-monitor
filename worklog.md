...
Task ID: 26
Agent: main
Task: Добавить критическое правило: ВСЕГДА ИСКАТЬ В ДОКУМЕНТАЦИИ ПРОШИВКИ 2.7.15 ПЕРЕД написанием кода для устройств

Work Log:
- Проверены PROJECT_RULES.md и AI_PROMPT.md — слабое правило 4.1 не требовало обязательного поиска ДО написания кода
- Добавлено критическое правило 4.1 в PROJECT_RULES.md: «ВСЕГДА ИСКАТЬ ИНФОРМАЦИЮ В ДОКУМЕНТАЦИИ ПО ПРОШИВКЕ 2.7.15 ПЕРЕД НАПИСАНИЕМ ФУНКЦИЙ ОТНОСЯЩИХСЯ К УСТРОЙСТВАМ!!!»
- Добавлено то же правило в AI_PROMPT.md (секция 6)
- Обновлены даты

Stage Summary:
- Правило добавлено в PROJECT_RULES.md (секция 4.1) и AI_PROMPT.md (секция 6)
- Причина: BT PIN баг (mode=0 вместо 1) возник именно из-за написания кода без проверки документации

---
Task ID: 27
Agent: main
Task: Фикс BT fixed PIN — исследована документация прошивки 2.7.15 и исправлен код моста

Work Log:
- Исследована документация прошивки 2.7.15 (protobuf config.proto, Python library, firmware source)
- Найдена корневая причина: BluetoothConfig.PairingMode enum — RANDOM_PIN=0 (ДЕФОЛТ), FIXED_PIN=1, NO_PIN=2
  - Код уже имел правильное значение mode=1, но были другие проблемы
- Добавлен импорт config_pb2 для использования protobuf-констант вместо magic numbers
- Добавлен ensureSessionKey() ПЕРЕД beginSettingsTransaction() — без session_passkey устройство МОЛЧА игнорирует admin-команды
- Добавлена диагностика BT после writeConfig("bluetooth") — чтение mode/pin/enabled обратно
- Добавлена диагностика BT после перезагрузки — проверка, что FIXED_PIN реально применился
- Добавлен BT_MODE_MAP для прямого маппинга (строка → enum)
- Добавлены предупреждения о известных проблемах nRF52840 (GitHub #9812, #7103)
- Обновлены: PROJECT_RULES.md, AI_PROMPT.md, STARTUP.md, techo-dump-config.py, techo-bridge.py

Stage Summary:
- Ключевые исправления: ensureSessionKey() перед транзакцией, protobuf-константы config_pb2, диагностика BT после записи/перезагрузки
- Известные проблемы nRF52840: FIXED_PIN может не работать (#9812), перезагрузка при BT-сопряжении (#7103)
- T-Echo имеет экран, поэтому #9812 может не проявляться — нужно тестировать на реальном устройстве
